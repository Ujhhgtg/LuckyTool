...
object ShowManualLockButtonPowerMenu : YukiBaseHooker() {
    override fun onHook() {
        if (SDK >= A14) loadHooker(ManualLockButton) else loadHooker(ManualLockButtonV13)
    }
    ...
    object ManualLockButtonV13 : YukiBaseHooker() {
        @Suppress("SameParameterValue")
        override fun onHook() {
            //Source ShutdownViewControl
            "com.oplusos.systemui.controls.ShutdownViewControl".toClass().apply {
                method { name = "initManuallyLock" }.hook {
                    after {
                        val context = args().first().cast<Context>() ?: return@after
                        val manuallyLockCanBeSeen = method { name = "manuallyLockCanBeSeen" }
            ...
            //Source GlobalActionsDialog
            "com.android.systemui.globalactions.GlobalActionsDialog".toClass().apply {
                method { name = "showOrHideDialog" }.hook {
                    after {
                        if (FlavorOneFeatureUtils(appClassLoader).isFlavorOneDevice() == true) return@after
                        val mExt = field { name = "mExt" }.get(instance).any() ?: return@after
            ...
            "com.oplusos.systemui.controls.GlobalActionsUtils".toClass().apply {
                method { name = "isSupportManuallyLock" }.hook {
                    replaceToTrue()
                }
                method { name = "isShowManuallyLock" }.hook {
                    after {
                        val context = args().first().cast<Context>() ?: return@after
                        val isShowManuallyLock = getOplusManuallyLock()?.current()?.method {
                            name = "isShowManuallyLock";param(ContextClass)
                        }?.invoke<Boolean>(context) ?: return@after
                        result = isShowManuallyLock
                    }
                }
            }
        }
        ...
        private fun getOplusManuallyLock(): Any? {
            val oplusFlavorManager =
                "com.oplusos.systemui.common.flavor.OplusFlavorManager".toClass()
            val dependency = DependencyUtils(appClassLoader).get(oplusFlavorManager)
            return dependency?.current()?.method { name = "getOplusManuallyLock" }?.call()
        }
